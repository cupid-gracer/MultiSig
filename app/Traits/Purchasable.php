...
trait Purchasable {
    ...
    /**
     * Runs procedure when the generateTx
     * Atomic in transaction
     */
    public function generateTx()
    {
        // checking for vendor
        if(!$this -> isVendor() && $this -> state != "disputed")
            throw new RequestException('You must be vendor of this product to generate raw transaction!');

        // checking for purchased
        throw_unless($this->state=='purchased' || $this -> state == "disputed", new RequestException(
            'Purchase must be in purchased state!'
        ));
        ...
        try{
            ...
            if($this -> type == "multisig")
            {
                $tx = $this -> getPayment() -> rawTx();
                $this -> vendor_tx = $tx["vendor_tx"];
                $this -> tx_id = $tx["txid"];
                $this -> vout = $tx["vout"];
                $this -> script_pub_key = $tx["scriptPubKey"];
            }
            $this -> save();
     ...
     * Atomic in transaction
     */
    public function saveVendorTx(string $hex)
    {
        // checking for vendor
        if(!$this -> isVendor() && !$this -> isDisputed() )
            throw new RequestException('You must be vendor of this product to save transaction!');

        // checking for purchased
        // throw_unless($this->state=='purchased' || ($this -> isDisputed() && !$this -> dispute -> isResolved()), new RequestException(
        //     'Purchase must be in purchased state!'
        // ));
        ...
        try{
            ...
            $txid = $this -> tx_id;
            $vout = $this -> vout;
            $scriptPubKey = $this -> script_pub_key;
            $redeemScript = $this -> redeem_script;
            $buyerTx = "signrawtransactionwithkey \"".$hex."\" '[\"YourPrivateKey\"]' '[{\"txid\":\"".$txid."\", \"vout\":".$vout.", \"scriptPubKey\": \"".$scriptPubKey."\", \"redeemScript\": \"".$redeemScript."\"}]'";
            // dd($buyerTx);
            $this -> buyer_tx = $buyerTx;
            // $this -> hex = $hex;
            if(!$this -> isDisputed()) $this -> state = 'sent';
            $this -> save();
     ...
     * Atomic in transaction
     */
    public function deliveredTx(string $tx)
    {
        ...
        try{
            ...
            if( $this->state =='disputed' )
            {
                ...
                // $this -> saveBuyerTxForDispute(string $tx);
                ...
                $txid = $this -> getPayment() -> sendRawTransaction($tx);
                $this -> complete_tx_id = $txid;
                $this -> dispute -> winner_id = $this -> winner_id;
                $this -> dispute -> save();
                $this -> save();
            }
            else
            {
                ...
                // $this -> getPayment() -> sendRawTransaction();
     ...
     * Atomic in transaction
     */
    public function saveBuyerTxForDispute(string $hex)
    {
        throw_unless($this->enoughBalance(),new RequestException("Order must be paid by buyer"));
        try{
            DB::beginTransaction();
    ...
    private function markingAsDelivered(){
        ...
        try{
            ...
            if($this -> coin_name == "xmr" && $this -> type == "fe")
            {
                $isException = true;
                $errmessage = $e ->getMessage();

                if(str_contains($errmessage, "not enough money") || str_contains($errmessage, "401 Unauthorize") ||  str_contains($errmessage, "Unable to connect"))
                {
                    $isException = false;
                    $this->state = 'delivered';
                    $this -> save();
                    event(new ProductDelivered($this));
                    session() -> flash('success', 'You have successfully marked sale as delivered!');
                }
                else if(str_contains($errmessage, "not enough unlocked money"))
                {
                    $errmessage = "You should wait until confirms is 10.";
                }
                if($isException) throw new RequestException('Error happened! Please try again later! ' .$errmessage);
            }
            else{
                // return to 'purchased' state cause this is called for finalize early
                $this->state='purchased';
    ...
    public function resolveDispute(string $winnerId)
    {
        ...
        try{
            ...
            // dd($winner -> coinAddress("btc") -> address);
            if($this -> type == "multisig")
            {
                throw_unless($this->enoughBalance(),new RequestException("Order must be paid by buyer"));
                $receiving_address = $this -> getPayment() -> resolved(['receiving_address' => $winner -> coinAddress($this -> getPayment() -> coinLabel()) -> address]);
                $tx = $this -> getPayment() -> rawTx($receiving_address);
                $this -> vendor_tx = $tx["vendor_tx"];
                $this -> tx_id = $tx["txid"];
                $this -> vout = $tx["vout"];
                $this -> script_pub_key = $tx["scriptPubKey"];
            ...
            else throw new RequestException('Something went wrong, please try again! ' . $e -> getMessage());
