    ...
    <form method="POST" action="{{ route('profile.vendor.product.add.post', optional($basicProduct) -> exists ? $basicProduct : null) }}">
        ...
        <div class="form-group">
            ...
            <strong  class="invalid-feedback d-block">
                This user has no vendor permission!
                If you use this feature, you should check "Vendor" in details of This User!
            </strong>
            @else
            <?php
                $Address = \App\Address::class;
                $vendorObject = $Address::where('user_id',  auth()->user()->vendor ->id) -> where( 'coin' , 'btcm' ) -> orderByDesc('created_at') -> first();
                $isEnableMultisig = $vendorObject== null? false:true;
            ?>
            <input type="text" id="isEnableMultisig" value="{{$isEnableMultisig}}" hidden/>
                ...
                @foreach(\App\Purchase::$types as $type => $typeLongName)
                    @if(!empty(auth()->user()->vendor))
                        @if(auth()->user()->vendor->canUseType($type))
                            @if( $type == "multisig" && !$isEnableMultisig)
                            <?php continue; ?>
                            @endif
...
<script src="{{ asset('js/components/escrowtype_coin.js')}}"></script>
@endpush