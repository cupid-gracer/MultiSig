...
<div class="col-md-12 mt-5 py-2" id="dispute">
    @if($purchase -> isDisputed())
        ...
        @if(!$purchase -> dispute -> isResolved() && auth() -> user() -> isAdmin())
            ...
            @if($purchase -> type == "multisig" && $purchase -> isDisputed() && !$purchase -> dispute -> isResolved() )
            <form action="{{ route('profile.purchases.disputes.resolve', $purchase) }}" class="form-inline" method="POST">
            @else
            <form action="{{ route('profile.purchases.disputes.resolve', $purchase) }}" class="form-inline" method="POST">
            @endif    
                {{ csrf_field() }}
                ...
                @if($purchase -> type == "multisig" && $purchase -> isDisputed() && !$purchase -> dispute -> isResolved() )
                <button type="submit" class="btn btn-outline-primary">Generate TX & Resolve dispute</button>
                @else
    ...
    @elseif(!$purchase -> isDelivered() && !$purchase -> isCanceled() && $purchase -> enoughBalance() && ($purchase -> isBuyer() || $purchase -> isVendor()))
