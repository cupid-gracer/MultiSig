...
class VendorController extends Controller
{
    ...
    public function addShow(NewBasicRequest $request, Product $product = null)
    {
        ...
        try{
            ...
            if( in_array("multisig", $types) && !in_array("btcm", $coins) )
                throw new RequestException('If you use Multisignature Escrow, you should select BTC Multisig as coin at least');
            if( count($types) ==1 && in_array("normal", $types) && in_array("btcm", $coins) )
                throw new RequestException('In Normal Escrow can\'t use BTC Multisig as coin.');
            if( count($types) > 1 && in_array("fe", $types) )
                throw new RequestException('If you use Finalize Early Escrow, should select only "Finalize Early"');
            if(in_array("fe", $types) && in_array("btcm", $coins) )
                throw new RequestException('In Finalize Early Escrow can\'t use BTC Multisig as coin.');
     ...
     * Returns view for confirming sent
     *
     * @param generateTx $sale
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View\
     */
    public function generateTx(Purchase $sale)
    {
        try{
            $sale -> generateTx();
            session() -> flash('success', 'You have successfully Generates Raw Transaction!');
        }
        catch (RequestException $e){
            $e -> flashError();
        }
     ...
     * Returns view for confirming sent
     *
     * @param submitVendorTx $sale
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View\
     */
    public function submitVendorTx(Purchase $sale, Request $req)
    {
        try{
            $hex = $req->input('transaction');
            throw_if($hex == null, new RequestException('Please input Transaction signed!'));
            $sale -> saveVendorTx($hex);
            session() -> flash('success', 'You have submit successfully signed Transaction!');
        }
        catch (RequestException $e){
            $e -> flashError();
        }
