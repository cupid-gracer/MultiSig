...
class ProfileController extends Controller
{
    ...
    public function addToCart(NewItemRequest $request, Product $product)
    {
        try
        {
            $coin = $request -> coin;
            $type = $request -> type;
            // if( ($coin == "btc" && $type != "normal" && $coin == "btc" && $type != "fe"))
            //     throw_if(true, new RequestException('If you use BTC, Escrow type should be "Normal"!'));
            if (  ($coin == "btcm" && $type != "multisig")  )
                throw_if(true, new RequestException('If you use BTC MULTISIG, Escrow type should be "Multisignature"!'));

            if( ($coin == "btc" || $coin == "xmr") && $type == "multisig" )
                throw_if(true, new RequestException('In "Multisignature" type, you can\'t use '.strtoupper($coin).' coin!'));
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function markAsDeliveredTx(Purchase $purchase, Request $req)
    {
        try{
            $tx = $req->input('transaction');
            throw_if($tx == null, new RequestException('Please input Transaction signed!'));
            $purchase -> deliveredTx($tx);
            session() -> flash('success', 'You have submited successfully!');
        }
        catch(RequestException $e){
            $e -> flashError();
        }
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View\
     */
    public function submitDesputeTx(Purchase $purchase, Request $req)
    {
        try{
            $hex = $req->input('transaction');
            throw_if($hex == null, new RequestException('Please input Transaction signed!'));
            $purchase -> saveVendorTx($hex);
            session() -> flash('success', 'You have submited successfully signed Transaction!');
        }
        catch (RequestException $e){
            $e -> flashError();
        }
