...
class Purchase extends Model
{
    ...
    public function getCoinSumAttribute()
    {
        ...
        //     return number_format($this -> getSum(), 12);
    ...
    public function getCoinBalanceAttribute()
    {
        ...
        if($balance == -1)
            return 'unavailable';
        // if($this -> coin_name == "xmr")
        //     return number_format($balance, 12);
    ...
    {
        return $Address = \App\Address::class;
    }
    ...
    * @return string
    */
    public function getBuyerPubkey():array
    {
        $buyerPubkey  = $this -> addresses()::where('user_id',  $this -> buyer_id)  -> where( 'coin' , 'btcm' ) -> orderByDesc('created_at') -> first() -> address;
        $vendorPubkey = $this -> addresses()::where('user_id',  $this -> vendor_id) -> where( 'coin' , 'btcm' ) -> orderByDesc('created_at') -> first() -> address;
        $Admin = \App\Admin::class;
        $adminId = $Admin::allUsers()[0] -> id;
        $adminPubkey = "";

        foreach ( $Admin::allUsers()  as $admin )
        {
            $adminId = $admin -> id;
            if (   count($this -> addresses()::where('user_id',  $adminId) -> where( 'coin' , 'btcm' )->get())  > 0 )
            {
                $adminPubkey = $this -> addresses()::where('user_id',  $adminId) -> where( 'coin' , 'btcm' ) -> orderByDesc('created_at') -> first() -> address;
                break;
            }
        }
        return array($buyerPubkey, $vendorPubkey, $adminPubkey);
    }
}
